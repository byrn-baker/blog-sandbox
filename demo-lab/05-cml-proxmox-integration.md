...
> **Superseded in part.** Two things in this doc describe a plan that was not
> built as written:
>
> 1. The per-DC bridging model (`vmbr100`/`vmbr200`/`vmbr300`, one bridge per
>    DC) was never built. The lab runs a single VLAN-aware `vmbr0` with one
>    VLAN per point-to-point link.
> 2. The K3s and DNS hosts are **Proxmox VMs on `vmbr0`**, not EVE-NG nodes.
>    Steps 3 and 4 below say they live inside EVE-NG; that is wrong. EVE-NG runs
>    only the Arista vEOS fabric.
>
> What did hold is the two-emulator split: CML runs the 13 Cisco IOS-XE
> cat8000v nodes (core + CEs), EVE-NG runs the 15 Arista vEOS switches, and the
> two meet on shared Proxmox VLANs via CML external connectors and EVE-NG
> clouds. See [08-hypervisor-interconnect.md](08-hypervisor-interconnect.md) for
> the verified as-built mapping. The concept and resource-sizing sections here
> still hold.
...
- **CML** — SP core (Cisco IOS-XE): P routers, PE routers, Route Reflectors,
  BORDER1, and CE routers (13 cat8000v nodes total)
- **EVE-NG** — Arista datacenter fabric (vEOS): the 15 spines and leaves

The K3s, server, and storage hosts are not emulator nodes. They run as Proxmox
VMs directly on the host and attach to the fabric over shared VLANs (see
[08-hypervisor-interconnect.md](08-hypervisor-interconnect.md)).
...
## Step 4: Linux VMs (Proxmox, not EVE-NG)
...
mapping is in [08-hypervisor-interconnect.md](08-hypervisor-interconnect.md).
