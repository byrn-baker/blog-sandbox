...
root=pathlib.Path('/tmp/mtu-peers-'+datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ'));root.mkdir();print(root,flush=True)
code=r'''
import json,concurrent.futures,re,datetime
from nautobot.dcim.models import Device
from netmiko import ConnectHandler
names=['DCB-Leaf03','DCC-Leaf03','SPE1','SPE2','SPE3']
...
def run(name):
 ...
 try:
  ...
  with ConnectHandler(device_type='arista_eos' if eos else 'cisco_ios',host=str(d.primary_ip4.host),username=user,password=password,secret=password,conn_timeout=8,auth_timeout=10,banner_timeout=15) as conn:
   ...
   commands=['show lldp neighbors Ethernet10 detail'] if eos else ['show mpls forwarding-table','show ip vrf','show ip route vrf UNDERLAY 10.3.1.4']
   row['commands']={}
   for cmd in commands:
