...
code=code.replace("names=list(Device.objects.filter(role__name__in=['Border-Router','CE-Router','P-Router','PE-Router','Route-Reflector','Leaf','Spine']).values_list('name',flat=True))", "names=['DCA-Leaf01','DCC-Leaf01','CE1','RR1']")
a=code.index("   commands=['show interfaces'");b=code.index("   row['commands']={}",a)
code=code[:a]+'''   if eos:
    source='10.3.1.4' if name=='DCA-Leaf01' else '10.3.3.4'
    targets=['10.3.1.5','10.3.3.4'] if name=='DCA-Leaf01' else ['10.3.1.4']
    ...
    for target in targets:
     for size in [1422,1472,1496,7972]:
...
for line in p.stdout.splitlines():
 if line.startswith('{'):
  d=json.loads(line)
  if 'device' in d:
   (out/(d['device']+'-probes.json')).write_text(json.dumps(d,indent=2));print(json.dumps(d),flush=True)
