...
root=pathlib.Path('/home/ubuntu/cml-qemu-live/capture-dca-dcb-jumbo');root.mkdir(exist_ok=True)
...
def launch(host,mode,local,peer):
 cmd=shlex.join(['python3','-c',src,mode,local,peer,'49492','262144','125000','8960']);return subprocess.Popen(['ssh','-o','BatchMode=yes',host,cmd],stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
