...
rows=json.loads((root/'verified-plans.json').read_text());names=sys.argv[1:];assert names and len(set(names))==len(names)
selected=[]
for name in names:
...
run('deploy-'+'-'.join(names),'7c91d376-a0e9-4178-96fe-7b80954ced04',{'config_plan':selected,'fail_job_on_task_failure':True,'debug':False})
